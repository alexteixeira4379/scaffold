...
def upgrade() -> None:
    ...
    op.execute("CREATE UNIQUE INDEX uq_candidates_email_lower ON candidates ((LOWER(email)))")
    ...
    op.create_table(
        ...
        sa.Column("event_data", sa.JSON(), nullable=False, server_default=sa.text("(JSON_OBJECT())")),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.text("now()"), nullable=False),
